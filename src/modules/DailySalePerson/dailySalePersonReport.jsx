...
<div className="p-6">
      <div className="overflow-x-auto rounded-lg shadow">
        <table className="min-w-full divide-y divide-gray-200 bg-white">
          <thead className="bg-indigo-600 text-white">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-sm font-semibold">Sales Person</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {markets.map((market, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-900">{market.market.map((item)=>(item.name + ","))}</td>
                <td className="px-6 py-4 text-sm text-gray-900">{market.salePersons.map((item)=>(item.name +","))}</td>
                <td className="px-6 py-4 text-sm text-gray-900">{formatDate(market.date)}</td>
        ...
        {markets.length === 0 && (
          <div className="text-center py-4 text-gray-500 text-sm">No Sales  data found.</div>
        )}
      </div>
    </div>
...
export default DailySalePersonReport;